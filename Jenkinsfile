...
pipeline {
	...
	environment {
        ...
        TERRAFORM_CMD = 'docker run -w /app -v /root/terraform.tfvars:/app -v /var/lib/jenkins/workspace/git-terraform-jenkins-aws/terraform-jenkins-aws/:/app -e $AWS_ACCESS_KEY_ID -e AWS_SECRET_KEY_ID hashicorp/terraform:light'
        TF_IN_AUTOMATION = '1'
    }
    stages {
        stage('git-checkout') {
