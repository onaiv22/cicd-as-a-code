...
pipeline {
	...
	environment {
        ...
        TERRAFORM_CMD = 'docker run -w /app  -v /root/.aws:/root/.aws -v pwd:/app -v /var/lib/jenkins/workspace/git-terraform-jenkins-aws/terraform-jenkins-aws/:/app hashicorp/terraform:light'
    }
    stages {
        stage('git-checkout') {
            steps {
