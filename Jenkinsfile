...
pipeline {
	...
	AWS_ACCESS_KEY_ID = credentials('jenkins-aws-access-key')
        ...
        TERRAFORM_CMD = 'docker run -w /app -v /var/lib/jenkins/workspace/git-terraform-jenkins-aws/terraform-jenkins-aws/:/app -v /root/.aws:/root/.aws -e env.${AWS_ACCESS_KEY_ID} -e env.${AWS_SECRET_KEY_ID} hashicorp/terraform:light'
    }
    stages {
        stage('git-checkout') {
            steps {
